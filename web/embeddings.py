...
def fetch_Mine(id, format="dict", corpus="wiki", normalize=True, lower=False, clean_words=True):
    print('Mine Embeddings id = {}'.format(id))
    id = int(id)

    download_file = {
            "wiki": "http://localhost/xxxxx.zip",
    }

    embedding_file = {
      "wiki": {
          100031: "fp_100031/"+format+"_100031",
          300001: "fp_300001/"+format+"_300001",
          300005: "fp_300005/"+format+"_300005",
          300007: "fp_300007/"+format+"_300007_final",
          300008: "fp_300008/"+format+"_300008",
          300010: "fp_300010/"+format+"_300010",
      },
    }

    assert corpus in download_file, "Unrecognized corpus"
    assert id in embedding_file[corpus], "Not available dimensionality"
    
    folder = './images/'
    return load_embedding(path.join(folder, embedding_file[corpus][id]),
                           format=format,
                           normalize=normalize,
...
def fetch_GloVe(dim=300, corpus="wiki-6B", normalize=True, lower=False, clean_words=True):
    ...
    print(_get_dataset_dir("embeddings"))
    # sys.exit(0)
