...
class Embedding(object):
    ...
    @staticmethod
    def from_dict(d, keys=None):

        if keys is None:
            for k in d:  # Standardize
                d[k] = np.array(d[k]).flatten()
                #print(d[k], len(d[k]), type(d[k]))
            return Embedding(vectors=list(d.values()), vocabulary=Vocabulary(d.keys()))
        else:
            values = []
            d = d.toarray()
            for k in d:  # Standardize
                #values.append(np.array(k).flatten())
                values.append(np.array(k))
            return Embedding(vectors=values, vocabulary=Vocabulary(keys))
